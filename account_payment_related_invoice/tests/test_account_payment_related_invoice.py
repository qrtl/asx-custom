...
class TestAccountPaymentRelatedInvoice(TransactionCase):
    ...
    def test01_payment_with_invoice(self):
        account_id = self.product1.categ_id.property_account_income_categ_id
        ...
        invoice_out1.pay_and_reconcile(
            self.env["account.journal"].search([("type", "=", "bank")], limit=1), 200.0
        )
        payment = invoice_out1.payment_ids[0]
        self.assertEqual(invoice_out1.number, payment.related_invoice_number)
        ...
        if invoice_out2.state == "open":
            invoice_out2.assign_outstanding_credit(
