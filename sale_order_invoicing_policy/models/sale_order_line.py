...
class SaleOrderLine(models.Model):
    ...
    #          order_id.sale_order_invoice_policy
    #          Allowing the change of invoice policy on SO
    @api.depends('qty_invoiced', 'qty_delivered', 'product_uom_qty',
                 'order_id.state', 'order_id.sale_order_invoice_policy')
        ...
        for line in self:
            if line.order_id.state in ['sale', 'done']:
                if line.product_id.type == 'service':
                    ...
                    invoice_policy = line.order_id.sale_order_invoice_policy or \
