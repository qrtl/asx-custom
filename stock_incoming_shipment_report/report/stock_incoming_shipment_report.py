...
class StockIncomingShipmentReport(models.TransientModel):
    ...
    tracking_pro_num = fields.Char(
        string='Tracking / PRO Number',
    )
    seal = fields.Char(
        string='Seal',
    )
    ...
    expiration_date_edit = fields.Date(
    ...
    def _compute_date_fields(self):
        for line in self:
            ...
            if line.expiration_date_edit:
                line.expiration_date = line.expiration_date_edit.strftime(
                    date_format)
