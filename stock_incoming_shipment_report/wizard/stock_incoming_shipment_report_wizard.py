    ...
    def generate_stock_incoming_shipment_report_action(self):
        ...
        for move in moves:
            vals = {
                ...
                'carrier_code': move.picking_id.carrier_id and move.picking_id.carrier_id.name or False,
            }
            self.env['stock.incoming.shipment.report'].create(vals)
        return self.env.ref('stock_incoming_shipment_report.action_stock_incoming_shipment_report').read()[0]
